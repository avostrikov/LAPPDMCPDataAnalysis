...
f0 = 800e6
x = [0.10, 0.25, 0.5, 0.75]
Nplots = 100
precision = 1e-12

fontsize = {'axis_name': 12, 'axis_tick': 10, 'legend': 10,
		'subtitle': 14, 'title': 16}

pl.rcParams['xtick.labelsize'] = fontsize['axis_tick']
pl.rcParams['ytick.labelsize'] = fontsize['axis_tick']

def printTime():
        return '[' + str(datetime.timedelta(seconds=time.time()-startTime))[:9] + ']'
...
def fit(x, y, fitfunc, p0, sigma, A=None):
	if A is None:
		...
		errfunc = lambda p, x, y: (fitfunc(p, x) - y) + (1e6 - (fitfunc(p, x) - y)) * (np.abs(np.abs(p[0]) - np.abs(A)) >= 0.10 * np.abs(A))
		p1, success = opt.leastsq(errfunc, p0, args=(x, y))
		x2 = chi2(x, y, fitfunc, p1, sigma)
		return p1, x2
	...
	fitfunc2 = lambda p, x: p[0] * np.exp(- (x-p[1])**2 / (2*p[2]**2)) + p[3]
		...
		nf.append(np.zeros(Npulses))
			...
			if (len(tt) == 5) and (len(t) > 10):
				...
				Range = np.all([t<=pPeak[1]+3*np.diff(t)[0], np.abs(v)>=0.15*np.abs(pPeak[0]+pPeak[3])], axis=0)
				t, v = t[Range], v[Range]
                	        if len(t) >= 5:
					ni[channel][pulse] = np.where(time == t[0])[0][0]
					nf[channel][pulse] = np.where(time == t[-1])[0][0]
                        	        p0 = [pPeak[0]+pPeak[3], pPeak[1], pPeak[2]]
                                	p1, X2[channel][pulse] = fit(t, v, fitfunc, p0, par[channel][pulse][11])
					Range = np.all([t<p1[1], np.abs(v)>0.10*np.abs(p1[0]), np.abs(v)<0.85*np.abs(p1[0])], axis=0)
					t, v = t[Range], v[Range]
					if len(t) >= 5:
						p2, X2[channel][pulse] = fit(t, v, fitfunc, p1, par[channel][pulse][11], np.abs(p1[0]))
						A[channel][pulse] = p2[0]
						B[channel][pulse] = p2[1]
						C[channel][pulse] = np.abs(p2[2])
					else:
